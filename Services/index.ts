...
export const service = {
    data: new DataService(env,
        new MongoClient(config.get("database.url")), {
            clientEmail: config.get("google.clientEmail"),
            privateKey: config.get("google.privateKey"),
            scripts: config.get("google.script")
        }),
...
app.get("/:project/card/:number", async (req, res) => {
    try {
        ...
        const ids = versions ? versions.map((version) => ({ number: parseInt(req.params.number), version })) : [{ number: parseInt(req.params.number) }];
...
app.get("/:project/cards", async (req, res) => {
    try {
        ...
        const ids = (req.query.ids ? getQueryArray(req.query.ids as string | string[]) : []).map((id) => id.trim().split("@")).map(([number, version]) => ({ number: parseInt(number), version }));
