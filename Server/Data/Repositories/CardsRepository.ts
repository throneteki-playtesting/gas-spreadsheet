...
import { GASCreateCardsResponse, GASDestroyCardsResponse, GASReadCardsResponse, GASUpdateCardsResponse } from "@/GoogleAppScript/Controllers/CardsController";
...
class CardMongoDataSource extends MongoDataSource<Card> {
    public async create({ cards }: { cards: Card[] }) {
        ...
        await service.render.syncImages(cards);
        const results = await this.collection.insertMany(cards);

        logger.info(`Inserted ${results.insertedCount} values into card collection`);
    ...
    public async read({ matchers }: { matchers: { project: number, number?: number, version?: SemanticVersion }[] }) {
        ...
        logger.info(`Read ${result.length} values from card collection`);
    ...
    public async update({ cards }: { cards: Card[] }) {
        ...
        await service.render.syncImages(cards, true);
        ...
        logger.info(`Upserted ${results.upsertedCount} values into card collection`);
    ...
    public async destroy({ matchers }: { matchers: { project: number, number?: number, version?: SemanticVersion }[] }) {
        ...
        logger.info(`Deleted ${results.deletedCount} values from card collection`);
...
class CardGASDataSource extends GASDataSource<Card> {
    public async create({ cards }: { cards: Card[] }) {
        ...
        for (const [p, c] of groups.entries()) {
            ...
            const response = await this.post<GASCreateCardsResponse>(url, body);
            created += response.created;
            logger.verbose(`${created} card(s) created in Google App Script (${project.name})`);
        }
        return created;
    }
    ...
    public async read({ matchers }: { matchers: { project: number, number?: number, version?: SemanticVersion }[] }) {
        ...
        for (const [p, m] of groups.entries()) {
            ...
            const url = `${project.script}/cards${ids ? `?ids=${ids.join(",")}` : ""}`;

            const response = await this.get<GASReadCardsResponse>(url);
            const read = response.cards as Card[];
            cards.push(...read);
    ...
    public async update({ cards }: { cards: Card[] }) {
        ...
        for (const [p, c] of groups.entries()) {
            ...
            const response = await this.post<GASUpdateCardsResponse>(url, body);
            updated += response.updated;
            logger.verbose(`${updated} card(s) updated in Google App Script (${project.name})`);
        }
        return updated;
    }
    ...
    public async destroy({ matchers }: { matchers: { project: number, number?: number, version?: SemanticVersion }[] }) {
        ...
        for (const [p, m] of groups.entries()) {
            ...
            const response = await this.get<GASDestroyCardsResponse>(url);
            destroyed += response.destroyed;
            logger.verbose(`${destroyed} card(s) deleted in Google App Script (${project.name})`);
        }
        return destroyed;
    }
}
