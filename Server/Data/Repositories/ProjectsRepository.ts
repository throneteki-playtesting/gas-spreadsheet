...
class ProjectMongoDataSource extends MongoDataSource<Project> {
    public async create({ projects }: { projects: Project[] }) {
        ...
        logger.info(`Inserted ${results.insertedCount} values into project collection`);
    ...
    public async read({ codes }: { codes?: number[] } = {}) {
        ...
        logger.info(`Read ${result.length} values from project collection`);
    ...
    public async update({ projects }: { projects: Project[] }) {
        ...
        logger.info(`Upserted ${results.upsertedCount} values into project collection`);
    ...
    public async destroy({ codes }: { codes?: number[] } = {}) {
        ...
        logger.info(`Deleted ${results.deletedCount} values from project collection`);
